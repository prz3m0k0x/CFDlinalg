...
/*--- Default structures shared by other libraries ---*/
typedef struct fvm_vector;
typedef struct fvm_mat;
typedef  fvm_csm_mat;
typedef struct fvm_linalg_system;
...
{
    switch (err) {
        case CFD_OK: return "OK";
        case CFD_INVALID_DIMENSIONS: return "invalid dimensions";
        case CFD_ALLOCATION_FAILED: return "allocation failed";
        case CFD_NULL_POINTER: return "null pointer";
        case CFD_OUT_OF_RANGE: return "out of range";
        case CFD_NONEQUAL_DIM: return "matricies have wrong dimensions to be cast";
        case CFD_SYSTEM_HOMOGENITY_ERROR: return "system should be homogenous, but b vector was provided";
        case CFD_DIVISION_BY_ZERO: return "Division by zero occured";
...
#define CFD_ERROR(err, ...) \
    do { \
        ...
        if (sizeof(#__VA_ARGS__) > 1) fprintf(stderr, ": " __VA_ARGS__); \
        fprintf(stderr, "\n"); \
    } while (0)
...
#define MAT_DIM_COMPARE_ADD_NULL(A, B) do { \
...
#define MAT_DIM_COMPARE_ADD_INT(A, B) do { \
    if ((A)->num_rows != (B)->num_rows || (A)->num_cols != (B)->num_cols) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return 0; \
    } \
...
#define MAT_DIM_COMPARE_ADD_VOID(A, B) do { \
    if ((A)->num_rows != (B)->num_rows || (A)->num_cols != (B)->num_cols) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return; \
    } \
...
#define MAT_DIM_COMPARE_MULT_NULL(A, B) do { \
...
#define MAT_DIM_COMPARE_MULT_INT(A, B) do { \
    if ((A)->num_cols != (B)->num_rows) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return 0; \
    } \
...
#define MAT_DIM_COMPARE_MULT_VOID(A, B) do { \
    if ((A)->num_cols != (B)->num_rows) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return; \
    } \
...
#define MAT_VEC_DIM_COMPARE_MULT_NULL(A, v) do { \
...
#define MAT_VEC_DIM_COMPARE_MULT_INT(A, v) do { \
    if ((A)->num_cols != (v)->num_dim) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return 0; \
    } \
...
#define MAT_VEC_DIM_COMPARE_MULT_VOID(A, v) do { \
    if ((A)->num_cols != (v)->num_dim) { \
        CFD_ERROR(CFD_INVALID_DIMENSIONS); \
        return; \
    } \
...
#define MAT_DIM_COMPARE_ADD_NULL(m1, m2)                  \
    do {                                                  \
...
#define MAT_DIM_COMPARE_ADD_INT(m1, m2)                   \
    do {                                                  \
        if ((m1)->num_rows != (m2)->num_rows ||          \
            (m1)->num_cols != (m2)->num_cols) {          \
            CFD_ERROR(CFD_NONEQUAL_DIM);                  \
